...
- `monitor/curmudgeon/priority-queue.json` — decider is the primary writer per `decider.md` Step E; the human operator may also push directly via the Cowork session (operator pushes must set `pushed_by` to a string containing `"operator"`). All other agents are forbidden from mutating the queue. The `writers` field in the file itself enumerates the allowed writers.
