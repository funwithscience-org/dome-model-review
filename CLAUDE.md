...
| dome-analyst | 3-slot off-peak (`0 1,5,9 * * *` local = UTC 00/04/08); PEAK-SHIFT 2026-06-08, was 6 slots. Tool-local = UTC+1. | Opus | `analyst.md` | New WIN onboarding, deep-attack/holistic expansions, defense neutralization, fingerprints |
...
| dome-curmudgeon | 3-slot off-peak (`0 2,6,10 * * *` local = UTC 01/05/09); PEAK-SHIFT 2026-06-08, was 6 slots. Tool-local = UTC+1. | Opus | `curmudgeon.md` | Adversarial self-review; change-driven + holistic reviews; discovery-mode (PROP-038) |
| dome-curmudgeon-verify | 3-slot off-peak (`30 1,5,9 * * *` local = UTC 00:30/04:30/08:30); PEAK-SHIFT 2026-06-08, was 6 slots. Tool-local = UTC+1. | Sonnet | `curmudgeon-verify.md` | Narrow verification of patched reviews (PROP-038 Phase 1) — class='verification' items with ≤2 minor prior holes |
...
| dome-decider | 3-slot off-peak (`0 3,7,11 * * *` local = UTC 02/06/10); PEAK-SHIFT 2026-06-08, was 6 slots. Tool-local = UTC+1. | Opus | `decider.md` | Triage, patches, new WIN commits, expansion integration |
...
| dome-workspace-sync | Hourly during active window only (`0 0-12,22-23 * * *` local = UTC 21-23 + 0-11); PEAK-SHIFT 2026-06-08, 15 runs/day (was 24). Skipped UTC 12-20 (= local 13-21) when no pipeline runs are active. Tool-local = UTC+1. | Haiku | `workspace-sync.md` | Commits workspace-only files to git |
| dome-mirror | Hourly at :30 during active window only (`30 0-12,22-23 * * *` local); PEAK-SHIFT 2026-06-08, 15 runs/day (was 24). Skipped UTC 12-20 (= local 13-21) when pipeline isn't active. Tool-local = UTC+1. | Haiku | `dome-mirror.md` | git→FUSE propagation (PROP-074, 2026-06-01) — runs `monitor/scripts/sync-workspace-step4c.js`; companion to workspace-sync |
