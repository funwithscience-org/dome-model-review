...
**2026-05-21 workspace-sync mass-delete incident (current state — agents PAUSED):** workspace-sync's Haiku LLM improvised a "no-checkout clone + mtime-only guard" fallback under disk pressure (sessions FS 100%, root FS 95%). The fallback interpreted unchecked-out files as deletions, producing commit `ea785c49` with stats +274 / -14,904,949 lines that wiped 4,733 of 4,755 tree entries from `origin/main`. 36 partial-recovery batches that fired automatically over the next ~15 minutes restored only ~3,309 entries, leaving 194 critical files (build.js, CLAUDE.md, all build-scripts/, README.md, .gitignore, .github/, web fonts) and 104 modifieds behind. Operator + cowork-claude force-reset `origin/main` to pre-disaster `1d256277` + re-pushed 2 genuinely-new poller/curmudgeon files atop, restoring all 4,757 entries at commit `1488170d`. **All 11 dome scheduled tasks remain DISABLED** until PROP-051 (workspace-sync safety + clone hygiene + integrity pruning, status `design-pending-operator-review`) lands. See `monitor/tinker/operator-directives/DIRECTIVE-20260521-001-workspace-sync-disaster-postmortem.json` for the postmortem and `monitor/tinker/proposals/PROP-051-workspace-sync-disaster-fix.json` for the 14-step unpause checklist. A secondary directive `DIRECTIVE-20260521-002-audit-files-no-round-trip.json` is also pending tinker (medium priority) — covers the related finding that audit-files write to git but don't round-trip to FUSE.
...
| dome-sloppytoppy-score | Daily 03:30 UTC | Sonnet | `sloppytoppy-score.md` | Readability scoring (PROP-039 Phase 1) — two-axis rubric (length + understandability) for flat-earth-level reader. |
| dome-sloppytoppy-rewrite | Every 2 days at 05:00 UTC (`0 5 */2 * *`) | Opus | `sloppytoppy-rewrite.md` | Readability rewriter (PROP-041 Phase 2) — drafts RW-NNN.json proposals for below-floor surfaces with first-class content-preservation audit. Propose-only; decider integrates after audit-script + curmudgeon-on-rewrite verification (class='rewrite-verify'). |
