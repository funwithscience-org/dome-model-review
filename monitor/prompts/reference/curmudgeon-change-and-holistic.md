...
4. **Prioritize.** Among flagged items, prefer: verdict changes > large text rewrites > new items > TLDR-only changes. **Within the 'large text rewrites' bucket, prefer the item with the largest drift magnitude** — magnitude reflects how much the prior review needs reconciliation with the current text, and items with extreme drift (e.g. detail_verdict_length grew +838% vs. +27%) signal more accumulated reconciliation debt than freshly-patched items with small recent drift. If multiple items still tie after magnitude ranking, prefer WINs over sections (WINs are what readers see first). When you record the chosen item's `change_summary` in the new review, name the field that drove the flag (e.g., "detail_verdict_length grew from 123 to 1154 (+838%); pre-tldr-schema fingerprint, compared on common fields only") so future audits can verify the catch rate.
