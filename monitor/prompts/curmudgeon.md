...
**Rewrite-verify branch (PROP-041, landed 2026-05-XX):** <!-- PROP-041 --> If `item.class === 'rewrite-verify'` AND `item.target_type === 'rewrite-proposal'`, you are reviewing a sloppytoppy rewrite proposal. This is your scope (NOT curmudgeon-verify's — Sonnet narrow cannot do the structural-judgment work). The procedure is different from the standard WIN review:

1. **Read the rewrite rubric.** `cat ${CLEAN_CLONE}/monitor/prompts/reference/sloppytoppy-rewrite-rubric.md` — this is your checklist for RWR-1..9.
2. **Read the RW file.** `cat ${CLEAN_CLONE}/${item.context_hints.rw_file}` — the proposal.
3. **Apply RWR-1..9** per the rubric. Each check produces a checklist_results entry. Decision thresholds: 0 major + 0 moderate + 0–2 minor → approve. 1+ major OR 2+ moderate OR 3+ minor → reject.
4. **Write the review** to `${CLEAN_CLONE}/monitor/curmudgeon/reviews/<RW-ID>-rewrite-verify.json` with `agent_subtype: 'curmudgeon-rewrite-verify'`. Include `queue_id` and `queue_pushed_at` per PROP-009 discipline.
5. **Update RW status** to either 'approved' (if rubric passed) or 'rejected' (if rubric failed). Add `curmudgeon_review_ref: 'monitor/curmudgeon/reviews/<RW-ID>-rewrite-verify.json'` to the RW. Commit both the review file AND the RW status update in the same commit.
6. **STOP.** One rewrite-verify item per run; this is singleton (rewrite-verify items are NOT batchable under Step 8a — content-preservation audit is too high-stakes for batch contamination).

The rewrite-verify branch does NOT use the standard WIN review template (no `claim`/`finding`/`detail_evidence` attack, no code_analysis tags, no advocate-mode). It uses the checklist output template documented in `sloppytoppy-rewrite-rubric.md`'s "Output template" section. Skip to Step 0c after writing the review.
...
5. **Pre-write staleness check.** Before writing the review, your clone may be 15-30 min old and the decider may have patched content in parallel. Run `git pull --rebase -q` in your clone, then re-read the target fields (e.g., wins.json for the WIN you reviewed, sections.json for the section). For each hole / finding in your draft review:
   ...
   **5a. Per-carry-over hole hygiene (added per PROP-047, 2026-06-14).** A carry-over hole is any draft hole whose description begins with `Carry-over from c[N-1]`, `Carried forward from c[N-1]`, `still unresolved from c[N-1]`, or maps to a `unresolved_prior_cycle[]` entry. For EACH carry-over hole in your draft holes_found:

   - **Phrase-presence re-grep.** Identify any verbatim phrase the c-N hole quoted from the data field (e.g., 'See Part 6.13', 'appended to our Pass band', 'toroidal sump node'). Grep current `data/wins.json` / `data/sections.json` for that phrase. If the phrase is ABSENT from current data, the carry-over hole is stale: rewrite the hole as 'Resolved between c-N and c-N+1 — quoted phrase "X" is no longer present in the cited field as of git HEAD <sha>' OR drop the hole entirely. Do NOT carry forward a hole text that claims a phrase exists when it does not.

   - **Closed-issue time-window filter.** Read `monitor/decisions/closed-issues.json`. Filter to issues where `fixed_at` is between c-N.reviewed_at and now AND `target_id` (or `description`) references the current review target. For each such ISS, compare its description to the c-N hole. If the ISS scope overlaps the c-N hole, the carry-over hole MUST disclose the disposition: either acknowledge resolution (preferred — rewrite to 'Carry-over verification: ISS-NNNN closed YYYY-MM-DD addressed this; re-verified by grep — no longer present'), or explain why the ISS patch is insufficient (legitimate persistence — e.g., 'ISS-NNNN patch did not address sub-defect X, which remains in the current field text — see grep evidence').

   - **Why this matters.** Without the per-hole re-grep, c-N+1 holes can copy c-N text verbatim and assert 'still unresolved' even when an intervening ISS has fixed the cited phrase. This costs analyst-Opus cycles downstream when the stale hole routes to expansion-tracker. Observed instance: WIN-055.c4 (2026-05-17T22:55Z) carried forward c3 hole #1 text describing a 'See Part 6.13' cross-reference inside sub_rationale, but ISS-2025 (fixed 2026-05-16T17:24Z, 29.5h earlier) had removed that cross-reference. The c4 hole-#1 text remained as if unchanged. Pre-write phrase-grep would have caught this in ~1 second.
...
1. **Queue item classified `class: 'verification'`.** Read `item.class` directly from the queue entry. PROP-025 (landed 2026-05-10) introduced this field as a content-aware classifier. The class is set at push time by whoever creates the work — analyst on EXPs (via `review_class` on the EXP file, propagated by decider at integration), decider on self-applied patches and prediction-batch integrations, operator on manual queue pushes. Allowed values: `'verification'` (batchable here), `'deep-attack'` (singleton — fresh adversarial argumentation expected), `'holistic'` (singleton — cross-WIN/cross-section), `'rewrite-verify'` (PROP-041 — singleton; content-preservation audit is too high-stakes for batch contamination). **Backward-compat:** if `item.class` is absent or null (legacy un-classed items pushed before PROP-025 landed), treat as `'deep-attack'` — singleton. Do NOT fall back to severity-from-closed-issues; that approach was rejected because it conflates "the original ISS was big" with "the remaining work is big," which are different things.

2. **No verdict change recommended.** If the queue item's `reason` text mentions verdict shifts ("verdict change", "Std Model → Refuted", "promoted to Self-Contradicted", etc.) or if the most recent review on this `target_id` had `verdict_change_recommended: true`, this is by definition not minor. Do not batch. (Note: a verdict-changing item should never carry `class: 'verification'` — if you encounter that combination, treat as singleton AND flag the misclassification in your `summary_for_decider`.)

3. **Combined batch byte budget within class tier.** Estimate each item's diff size from its `reason`. Cap depends on class:
   - `class: 'verification'` items: combined batch up to **~20 KB** of new text to read. (Raised from 10 KB by PROP-025; covers realistic 3-item verification batches — typical EXP-integration diffs 1-3 KB, patch-self-apply diffs 200-2000 bytes, worst-case heavy refinement EXPs 5-7 KB.)
   - `class: 'deep-attack'` and `class: 'holistic'` items: not batchable per gate 1; cap is moot.

   If adding the next item would push the combined batch beyond ~20 KB, stop adding. (The previous "EXP integration NOT batchable, regardless of severity" carve-out has been removed — class-based discrimination handles this directly: an EXP integration that introduced a new argument is `class: 'deep-attack'` (singleton); an EXP that refined existing prose is `class: 'verification'` (batches).)
...
5. **Item must not be a fresh onboard.** `target_type: win-new`, `target_type: section-new`, or `target_type: proposal` are foundational and always singleton. Batching applies to re-reviews of already-reviewed targets only (`win-detail-rewrite`, `section-rewrite`, `killshot-rewrite`). (Defense in depth: these target types should also be pushed with `class: 'deep-attack'` per PROP-025's decider-population matrix — gate 1 already singles them out — but gate 5 stays as belt-and-braces.)
...
**Operator-commissioned holistic-rerun override (added 2026-05-28).** Before delegating to the reference, check `tracker.holistic_checks[]` for any entry where `status === 'pending'` AND the entry has a top-level `operator_phase2_rerun_commission` field present (non-null). If any such entry exists, **REVERSE this run's normal Priority 4 ↔ Priority 5 order**: execute holistic review (Step 0e) FIRST, falling through to change-driven (Step 0d) only if Priority 5 produces no work this run (no pending HOL eligible). This is a temporary priority inversion the operator commissioned to drain a backlog of holistic checks during a content-stable window. The clause is **self-clearing**: when every commissioned HOL has been reviewed (status flipped to `reviewed` by your normal tracker-update behavior), the condition no longer holds and the natural Priority 4 → 5 → 6 order resumes automatically. No prompt-rollback edit is required when the rerun completes. The override does NOT bypass Priority 0a/0b/0c/0c2 — priority queue, human notes, and major-change audit still pre-empt as usual.

→ Read `monitor/prompts/reference/curmudgeon-change-and-holistic.md` and execute the first applicable procedure (change detection → holistic review → spot-check), with the operator-commission override applied when in effect (holistic FIRST, then change detection if no eligible HOL remains this run).
...
4. **Change-driven review** (Step 0d) — content that changed since last review *(swapped below 5 when operator-commission override is active)*
5. **Holistic review** (Step 0e) — periodic broad review of the whole document *(promoted above 4 when any pending HOL has `operator_phase2_rerun_commission` tag)*
