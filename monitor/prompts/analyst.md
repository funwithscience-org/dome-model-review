...
if [ "${__SESS_AV:-0}" -lt 700 ] && [ "${__ROOT_AV:-0}" -ge 1000 ]; then
  CLEAN_CLONE="/tmp/dome-review-clean"
  ...
  # FAIL-CLOSED: write monitor/integrity/analyst-abort-<ISO>.json with
...
Outcomes:
- If you find a real issue → write an issue-proposal file to `monitor/analyst/issue-proposals/proposal-mode6-<slug>-<epoch>.json` (same schema as change-analysis issue-proposals; include a `mode6_target` provenance block). The decider drains this directory every run (Priority 1b, ledger-deduped) and routes the resulting ISS through curmudgeon like all other work. **NEVER author a self-initiated EXP file from Mode 6 (PROP-144, ISS-3033):** the decider has no intake path for analyst-self-initiated expansions — Mode-6 EXP files strand unallocated (EXP-671/673/674/675 sat 8-10 days) and their file IDs collide with tracker next_id allocations because the analyst is READ-ONLY on the tracker. If the finding genuinely needs an expansion, say so in the issue-proposal and let the decider assign it back with a properly-allocated EXP id.
...
- **NARRATE-CITE discipline (Mode 1b verdict prose, PROP-014 Mech 3):** every paragraph >1 sentence in `detail_reasoning` and `tldr` MUST contain at least one inline `(file.json:anchor)` or source URL citation. The verdict prose is a state-bearing claim about dome's prediction performance — cite the specific dome source URL, the dome predictions.json entry, the WIN-NNN it relates to, or the standard-physics paper you're invoking. Don't narrate from prompt-chain memory of similar prior writeups. See `monitor/prompts/reference/state-verification.md` Discipline 3. The audit script `monitor/scripts/audit-narrative-citations.js` (TBA) will fail uncited paragraphs in declared-state surfaces.
