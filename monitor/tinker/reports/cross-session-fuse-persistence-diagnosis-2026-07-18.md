...
Most parsimonious reconstruction: the missing-files observation was made **in the operator's morning cowork session** (the one that ran the 07:15Z off-schedule sync), against the operator-side (iCloud-backed) view of the folder, and was later attributed to the tinker run when the directive was drafted. The mtime evidence fits iCloud materialization mechanics: files evicted or not-yet-materialized on the Mac are invisible/erroring through the bash bridge (a documented caveat of the cloud-synced mounts), and re-materialization preserves original mtimes — which is exactly the signature observed (files "reappear" with old mtimes, nothing carries a 07:15Z copy timestamp).

**Evidence limit, stated plainly per the directive's no-speculation constraint:** repo data cannot prove what the morning session saw or why. The iCloud-materialization account is the best fit to the observable signature (correct same-day mtimes + `new_files=9` not re-copying them + zero trace of the observation in any agent artifact), but it is a reconstruction, not a measurement.
...
- **OPT-4 (platform escalation) — nothing to escalate.** Scheduled-session persistence works. The residual softness is operator-side iCloud materialization visibility, already documented as a caveat of the cloud-synced mounts.

**Shipped with this run (per part_C_ship_or_defer, if_OPT_2):** a CLAUDE.md addendum documenting (a) the verified persistence result with its evidence signature, so no future session re-derives it; (b) the transient-visibility class (iCloud materialization) and the clone-based-read fallback when FUSE looks impossibly stale; and (c) an attribution-hygiene rule: before quoting an agent artifact in a directive or issue, grep the artifact for the quote — this incident's directive cites a sentence that exists nowhere.
