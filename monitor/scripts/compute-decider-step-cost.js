 ...
 *
 * Usage:
 *   node compute-decider-step-cost.js <t1.jsonl> [t2.jsonl ...] [--threshold N]
 *
...
// PROP-140 signature table — decider step-entry commands. A match on the
...
// EXTEND THIS TABLE when decider steps change: each entry is
...
const SIGNATURES = [
  [/digest-reviews\.js/,        'curmudgeon-digest'],
  [/allocate-iss-ids\.js/,      'issue-mint'],
  [/lint-decider-surfaces\.js/, 'pre-push'],
  [/audit-rewrite\.js/,         'rewrite-audit'],
  [/build\.js publish/,         'publish'],
  [/sync-workspace/,            'workspace-sync-step'],
  [/stranded-patches-/,         'patch-strand'],
  [/priority-queue\.json/,      'queue-mgmt'],
];
...
function parseCli() {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--threshold' && argv[i + 1]) { threshold = parseInt(argv[i + 1], 10); i++; }
      ...
      console.error('usage: compute-decider-step-cost.js <t1.jsonl> [t2.jsonl ...] [--threshold N]');
      process.exit(0);
...
function main() {
  ...
  for (const t of transcripts) {
    if (!fs.existsSync(t)) {
      console.error(`compute-decider-step-cost: ${t} not found; skipping`);
      continue;
    }
    let lines;
    try {
      ...
      console.error(`compute-decider-step-cost: failed to read ${t}: ${e.message}; skipping`);
      continue;
    }
    segments.push({ file: t, lines, firstTs: segmentFirstTs(lines) });
  }
  if (!segments.length) {
    console.error('compute-decider-step-cost: no readable transcript among supplied paths');
    process.exit(1);
  }
  segments.sort((a, b) => {
