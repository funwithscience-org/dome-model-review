#!/usr/bin/env node
/*
 * compute-decider-step-cost.js (PROP-104 Phase 0, 2026-06-17)
 *
 * Per-step token attribution for decider runs. Read-only transcript
 * analyzer; mirrors compute-run-cost.js but buckets usage by STEP_MARKER
 * stderr breadcrumbs that decider emits at each dispatcher step.
 *
 * Mechanism A (per-step input/cache/output bucketing):
 *   Decider emits `echo "STEP_MARKER <step> $(date +%s)"` at the start
 *   of each major dispatcher step (Step A0/A0b/A0c/A0d, Priority 1b/2/3/
 *   4/5b, etc). The markers land in bash tool_result stderr and so are
 *   readable in the transcript. This analyzer aligns each assistant
 *   message's usage with the active step window (last marker seen
 *   before that message), then sums input / cache_read / cache_write /
 *   output per window.
 *
 ...
 *
 * Usage:
 *   node compute-decider-step-cost.js <transcript.jsonl> [--threshold N]
 *
...
function parseCli() {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === '--threshold' && argv[i + 1]) { threshold = parseInt(argv[i + 1], 10); i++; }
      ...
      console.error('usage: compute-decider-step-cost.js <transcript.jsonl> [--threshold N]');
      process.exit(0);
    } else if (!transcript) { transcript = a; }
  }
  if (!transcript) {
    console.error('compute-decider-step-cost: missing transcript path');
    console.error('usage: compute-decider-step-cost.js <transcript.jsonl> [--threshold N]');
    process.exit(1);
  }
...
function main() {
  ...
  if (!fs.existsSync(transcript)) {
    console.error(`compute-decider-step-cost: ${transcript} not found`);
    process.exit(1);
  }
  let lines;
  try {
    ...
    console.error(`compute-decider-step-cost: failed to read ${transcript}: ${e.message}`);
    process.exit(1);
  }
