...
const OWNERSHIP = {
  ...
  // PROP-021 Phase 1 (2026-06-14): curmudgeon dispatcher-state precompute.
  // The script is git-owned source code (same as compute-drift-audit.js
  // pattern from PROP-020). The output artifact lives in monitor/integrity/
  // and is regenerated by the script — classified 'git' so build.js publish
  // ferries it git→workspace whenever the script runs in a clone (e.g.
  // integrity's §7i refresh, or the Phase 2 curmudgeon-side bash refresh
  // once curmudgeon.md consumes the artifact). Workspace-sync NEVER pushes
  // it FUSE→git (entry mirrored in workspace-sync.md NEVER_PUSH).
  'monitor/scripts/compute-curmudgeon-dispatcher-state.js': 'git',
  'monitor/integrity/curmudgeon-dispatcher-state.json': 'git',

  // PROP-111 Phase 1 1A (2026-06-20): integrity mechanical-checks precompute.
  // The script is git-owned source code, output artifact lives in
  // monitor/integrity/ and is regenerated by the script at the top of every
  // integrity cycle (see structure-integrity.md §0). Classified 'git' so
  // build.js publish ferries it git→workspace whenever the script runs in a
