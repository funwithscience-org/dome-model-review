...
<p><strong>Credit where due:</strong> The dome model does not use a naive flat-earth azimuthal equidistant projection (which would give ~8,300 km — wildly wrong). It uses a custom V13 Finsler coordinate system with a two-zone southern hemisphere topology, an elliptic integral for east-west arc lengths, and a position-dependent aetheric refractive index n(r) that adjusts distances based on radial position. This system does produce 4,352 km for Sydney-Perth. That is substantially more mathematical sophistication than typical flat-earth models, and the 4,352 km figure is derived from the model's own formulas — not simply copied from a railway timetable.</p>

<p><strong>The problem — which distance are we testing against?</strong> The Indian Pacific railway runs Sydney → Broken Hill → Adelaide → Cook → Perth, a route that detours ~1,061 km south through Adelaide. Its 4,352 km distance is a function of 19th-century railway routing, not Earth's geometry. The globe geodesic (shortest surface path) is 3,291 km, confirmed by direct flight paths (~3,290 km) and satellite measurement. The driving distance is ~3,935 km. A geometric model should be tested against the geometric distance between two points — not a circuitous infrastructure route.</p>

<p><strong>The deeper problem — the model's own admitted failures:</strong> The dome's coordinate page lists its own target: "SYD–EZE error &lt;5% (currently −73%)." That means for Sydney to Buenos Aires (two cities at similar southern latitudes, separated by more longitude), the same Finsler formula that produces 4,352 km for Sydney-Perth is <strong>off by 73%</strong>. The aetheric refractive index n(r) is a free function with no independent measurement — it can be tuned to match one city pair while failing catastrophically on another. This is the same unfalsifiability problem we identify with aetheric refraction in optics (see <a href="#part4b" onclick="showTab('selftest');return false">Internal Contradictions</a>): a correction factor with no independent constraint can match any individual data point but has no predictive power across multiple tests.</p>

<p><strong>Summary:</strong> The dome's Finsler coordinate system produces 4,352 km for Sydney-Perth — close to the railway distance but 32% above the geometric distance (3,291 km). Direct flights confirm the globe geodesic. More importantly, the same formula admits 73% error on SYD-EZE, revealing that the aetheric index n(r) is curve-fitted to specific routes. A model that matches one southern hemisphere distance while failing another by 73% has not demonstrated predictive power — it has demonstrated overfitting.</p>
...
<p><strong>The dome model:</strong> Uses a V13 Finsler coordinate system with a two-zone southern hemisphere topology, elliptic integral arc lengths, and a position-dependent aetheric refractive index n(r). This is substantially more sophisticated than naive flat-earth distance calculations. The question is whether it produces accurate distances across multiple southern hemisphere city pairs.</p>
<p><strong>The test the model fails on its own terms:</strong> The dome's own coordinates page reports its Sydney–Buenos Aires (SYD–EZE) distance error as <strong>−73%</strong>, with a stated target of bringing this below 5%. Both cities sit near 34°S — this is a pure east-west test at southern latitudes. The globe geodesic for SYD-EZE is ~11,800 km, confirmed by direct flights (~14 hours at ~840 km/h). A 73% error means the dome's formula produces roughly 3,200 km for a route that is actually 11,800 km. The model's own published data admits this failure.</p>
<p><strong>The pattern:</strong> The dome's Finsler formula matches Sydney–Perth (4,352 km predicted vs. 3,291 km geodesic — a 32% overshoot, but at least the right order of magnitude). It fails catastrophically on SYD–EZE (73% undershoot). The aetheric refractive index n(r) is a free function with no independent measurement, so it can be tuned to match specific routes while failing on others. This is overfitting, not prediction. A genuine geometric model should produce consistent accuracy across all city pairs at similar latitudes — not match one while being off by 73% on another.</p>
