...
function resolvePlaceholders(html, context) {
  ...
  const replacements = {
    ...
    // V51-cohort structural-failure counts (ISS-2432): of the new_in_v51 WINs,
    // how many have monitoring=none / post_hoc=true / derives_from_dome=false.
    '{{V51_NO_MONITOR}}': context.v51NoMonitor || 0,
    '{{V51_POST_HOC}}': context.v51PostHoc || 0,
    '{{V51_NO_DERIVE}}': context.v51NoDerive || 0,
...
function renderFailureEntry(e) {
  ...
  const refLabel = e.dome_ref_current
    ? escapeHtml(e.dome_ref_current) + (e.dome_status_current ? ` <span style="color:var(--refuted-solid);font-weight:600;font-style:italic" title="Our tracking status, inferred from commit diffs and section-header movement. The dome does not attach per-entry bracket markers.">(our status: ${escapeHtml(e.dome_status_current.toLowerCase())})</span>` : '')
...
<p style="margin:.25rem 0"><strong>Effective status (our tracking label):</strong> <em>${domeLabel}</em>${evidenceDate ? ` <span style="color:#888;font-size:.85rem">(${evidenceDate})</span>` : ''}</p>
...
<p>${totalSilent} prediction${totalSilent === 1 ? '' : 's'} the dome has effectively disowned — silently removed from the wins/predictions page (observable via git commit diffs that delete the entry between snapshots) or grouped under a <code>SUSPENDED — Under Revision</code> section header — but excluded from both the headline confirmed count and the ${totalDome} acknowledged failures. The dome does not attach per-entry bracket markers; the disowning is inferred from absence-in-current-snapshot, section-header movement, and version-note acknowledgments in evolution.html. These are the entries that should reduce his accuracy but don't.</p>
...
function generateVerdictBarChart(tally, total, collisionCount) {
  ...
  const rows = ordered.map(verdict => {
    ...
    return `<a class="ds-verdict-bar-row" href="${anchor}" onclick="showTab('wins');return false" aria-label="${verdict}: ${count} of ${total} catalog entries (${pct}%).${ariaSuffix}">
...
.ds-falsifiability-module{margin:1.4rem 0 1.6rem;padding:0.95rem 1.05rem 1rem;border:1px solid var(--border);border-left:3px solid var(--accent);border-radius:6px;background:var(--card-bg)}.ds-falsifiability-module .ds-fm-heading{font-family:var(--sans);font-size:0.82rem;text-transform:uppercase;letter-spacing:0.06em;color:var(--ink-2);margin:0 0 0.2rem;font-weight:600;border:none;padding:0}.ds-falsifiability-module .ds-fm-caption{font-family:var(--sans);font-size:0.85rem;color:var(--ink-2);margin:0 0 0.7rem}.ds-falsifiability-module .ds-fm-caption .ds-fm-jump{color:var(--accent);text-decoration:none;font-weight:600}.ds-falsifiability-module .ds-fm-caption .ds-fm-jump:hover,.ds-falsifiability-module .ds-fm-caption .ds-fm-jump:focus-visible{text-decoration:underline}.ds-falsifiability-module .ds-fm-rows{list-style:none;margin:0;padding:0}.ds-falsifiability-module .ds-fm-row{display:grid;grid-template-columns:17rem minmax(0,1fr) 13rem;gap:0.55rem 0;align-items:baseline;padding:0.7rem 0;border-top:1px solid var(--border)}.ds-falsifiability-module .ds-fm-row:first-child{border-top:none;padding-top:0.3rem}.ds-falsifiability-module .ds-fm-condition{font-family:var(--sans);font-weight:600;color:var(--heading);font-size:0.95rem;line-height:1.35;text-wrap:balance;padding-right:1.1rem;border-right:1px solid var(--border)}.ds-falsifiability-module .ds-fm-desc{font-family:var(--serif);color:var(--ink-2);font-size:0.9rem;line-height:1.45;padding-left:1.1rem;padding-right:1.1rem;border-right:1px solid var(--border)}.ds-falsifiability-module .ds-fm-status{display:flex;flex-direction:column;align-items:flex-end;gap:0.1rem;justify-self:stretch;text-align:right;padding-left:1.1rem;width:100%}.ds-falsifiability-module .ds-fm-status-num{font-family:var(--sans);font-weight:700;font-variant-numeric:tabular-nums;font-size:1.2rem;line-height:1.15;color:var(--heading)}.ds-falsifiability-module .ds-fm-status-sub{font-family:var(--sans);font-size:0.72rem;color:var(--ink-2);text-transform:uppercase;letter-spacing:0.05em;line-height:1.25}.ds-falsifiability-module .ds-fm-status-zero .ds-fm-status-num{color:var(--refuted-solid)}.ds-falsifiability-module .ds-fm-status-partial .ds-fm-status-num{color:var(--accent)}.ds-falsifiability-module .ds-fm-status-pending .ds-fm-status-num{color:var(--ink-2)}@media(max-width:720px){.ds-falsifiability-module .ds-fm-row{grid-template-columns:1fr auto;grid-template-areas:"cond status" "desc desc";gap:0.3rem 0.6rem}.ds-falsifiability-module .ds-fm-condition{grid-area:cond}.ds-falsifiability-module .ds-fm-status{grid-area:status;align-items:flex-end;min-width:0}.ds-falsifiability-module .ds-fm-desc{grid-area:desc;font-size:0.86rem}.ds-falsifiability-module .ds-fm-status-num{font-size:1.1rem}}
.ds-hk-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1.2rem}@media(max-width:720px){.ds-hk-grid{grid-template-columns:1fr}}
.ds-sc-domains{grid-template-columns:repeat(3,1fr)}
.ds-thesis-hero{margin:1.6rem 0 1.4rem;padding:0;border:none}
...
.ds-thesis-hero .ds-thesis-badge-pre{text-transform:uppercase;font-size:.72rem;letter-spacing:0.08em;opacity:.85}
...
.ds-thesis-hero .ds-tf-textnum{font-size:1.05rem;font-weight:700;color:var(--accent);letter-spacing:0.02em;text-transform:none}
.ds-thesis-hero .ds-tf-accent .ds-tf-number{color:var(--accent)}
.ds-thesis-hero .ds-tf-label{font-size:.78rem;text-transform:uppercase;letter-spacing:0.07em;color:var(--ink-3);font-weight:600;line-height:1.25}
.ds-thesis-hero .ds-tf-link{font-size:.78rem;color:var(--link);font-weight:500;margin-top:.1rem;text-decoration:underline}
.ds-thesis-hero .ds-tf-link:hover{text-decoration:none}
@media(max-width:760px){.ds-thesis-hero .ds-thesis-figures{grid-template-columns:repeat(2,minmax(0,1fr));gap:.9rem}.ds-thesis-hero .ds-tf-number{font-size:1.35rem}}
@media(max-width:420px){.ds-thesis-hero .ds-thesis-figures{grid-template-columns:1fr}}
...
.ds-evaluate-preface{margin:1.4rem 0 1.6rem;padding:0.85rem 1.05rem 0.95rem;border:1px solid var(--border);border-left:3px solid var(--ink-3);border-radius:6px;background:var(--card-bg)}
.ds-evaluate-preface .ds-ep-heading{font-family:var(--sans);font-size:0.82rem;text-transform:uppercase;letter-spacing:0.06em;color:var(--ink-2);margin:0 0 0.2rem;font-weight:600;border:none;padding:0}
.ds-evaluate-preface .ds-ep-caption{font-family:var(--sans);font-size:0.85rem;color:var(--ink-2);margin:0 0 0.7rem}
.ds-evaluate-preface .ds-ep-caption .ds-ep-jump{color:var(--accent);text-decoration:none;font-weight:600}
.ds-evaluate-preface .ds-ep-caption .ds-ep-jump:hover,.ds-evaluate-preface .ds-ep-caption .ds-ep-jump:focus-visible{text-decoration:underline}
.ds-evaluate-preface .ds-ep-rows{list-style:none;margin:0;padding:0}
.ds-evaluate-preface .ds-ep-row{display:grid;grid-template-columns:1.5rem minmax(0,1fr) auto;gap:0.45rem 0.9rem;align-items:baseline;padding:0.5rem 0;border-top:1px solid var(--border)}
.ds-evaluate-preface .ds-ep-row:first-child{border-top:none;padding-top:0.2rem}
.ds-evaluate-preface .ds-ep-num{font-family:var(--sans);font-size:0.78rem;font-weight:700;color:var(--ink-2);font-variant-numeric:tabular-nums;text-align:right;line-height:1.4}
.ds-evaluate-preface .ds-ep-q{font-family:var(--serif);color:var(--heading);font-size:0.95rem;line-height:1.4;text-decoration:none;font-weight:500}
.ds-evaluate-preface .ds-ep-q:hover,.ds-evaluate-preface .ds-ep-q:focus-visible{color:var(--accent);text-decoration:underline}
.ds-evaluate-preface .ds-ep-where{font-family:var(--sans);font-size:0.72rem;text-transform:uppercase;letter-spacing:0.06em;color:var(--ink-2);font-weight:600;justify-self:end;text-align:right;line-height:1.3;white-space:nowrap}
@media(max-width:720px){.ds-evaluate-preface .ds-ep-row{grid-template-columns:1.4rem minmax(0,1fr);grid-template-areas:"num q" ". where";gap:0.2rem 0.6rem}.ds-evaluate-preface .ds-ep-num{grid-area:num}.ds-evaluate-preface .ds-ep-q{grid-area:q;font-size:0.92rem}.ds-evaluate-preface .ds-ep-where{grid-area:where;justify-self:start;text-align:left;font-size:0.7rem;white-space:normal}}
...
.adoption-label{margin:1.25rem 0 .5rem;padding:.5rem .85rem;background:var(--card-bg);border:1px solid var(--border);border-left:4px solid var(--accent);border-radius:3px;font-size:.95rem;line-height:1.45}
.adoption-label strong{color:var(--heading)}
...
function main() {
  ...
  const context = {
    ...
    // V51-cohort structural-failure subsets (ISS-2432): of new_in_v51 WINs,
    // count by monitoring=none, post_hoc=true, !derives_from_dome.
    v51NoMonitor: wins.filter(w => w.new_in_v51 && ((w.code_analysis?.monitoring || 'none') === 'none')).length,
    v51PostHoc: wins.filter(w => w.new_in_v51 && w.code_analysis?.post_hoc === true).length,
    v51NoDerive: wins.filter(w => w.new_in_v51 && !w.code_analysis?.derives_from_dome).length,
...
<li class="ds-tf-item">
<span class="ds-tf-number">${counts.total}</span>
<span class="ds-tf-label">Claimed wins</span>
</li>
<li class="ds-tf-item ds-tf-accent">
<span class="ds-tf-number">0</span>
<span class="ds-tf-label">Survive scrutiny</span>
</li>
<li class="ds-tf-item">
<span class="ds-tf-number">${failures.dome_claimed_failures}+${silentFailures}</span>
<span class="ds-tf-label">Acknowledged + silent failures</span>
<a class="ds-tf-link" href="#p3-failures" onclick="showTab('wins');return false">Details &rarr;</a>
</li>
<li class="ds-tf-item">
<span class="ds-tf-number ds-tf-textnum">Wrong side</span>
<span class="ds-tf-label">Timestamp error</span>
<a class="ds-tf-link" href="#timestamp-error" onclick="showTab('timestamp');return false">Details &rarr;</a>
</li>
</ul>
</section>
...
<div class="ds-fm-desc">Reproduce the Schumann fundamental from dome parameters alone &mdash; no globe radius, no spherical-cavity eigenvalue formula.</div>
<div class="ds-fm-status ds-fm-status-zero" role="status" aria-label="Status: 0 of ${counts.total} claims met; 1 attempted (WIN-001), imports the globe radius"><span class="ds-fm-status-num">0 / ${counts.total} met</span><span class="ds-fm-status-sub">1 attempted &middot; imports globe radius</span></div>
</li>
<li class="ds-fm-row">
<div class="ds-fm-condition">Sub-5% coordinates without globe inputs</div>
<div class="ds-fm-desc">Reproduce city-to-city distances from dome geometry alone &mdash; no WGS84 input, no curve-fitting against globe-measured distances.</div>
<div class="ds-fm-status ds-fm-status-zero" role="status" aria-label="Status: 0 of ${counts.total} claims met; the dome itself flags this as unsolved (OPEN-001)"><span class="ds-fm-status-num">0 / ${counts.total} met</span><span class="ds-fm-status-sub">OPEN-001 &middot; dome-acknowledged</span></div>
</li>
</ol>
</section>
...
<a class="ds-ep-q" href="#stdmodel" onclick="showTab('wins');return false">Does the prediction distinguish this model from alternatives?</a>
<span class="ds-ep-where">Standard Model Explains (3.4)</span>
</li>
<li class="ds-ep-row">
<span class="ds-ep-num" aria-hidden="true">2</span>
<a class="ds-ep-q" href="#selfcon" onclick="showTab('wins');return false">Can the prediction be derived from the model's own parameters?</a>
<span class="ds-ep-where">Self-Contradicted (3.3)</span>
</li>
<li class="ds-ep-row">
<span class="ds-ep-num" aria-hidden="true">3</span>
<a class="ds-ep-q" href="#refuted" onclick="showTab('wins');return false">Is the cited data accurately represented?</a>
<span class="ds-ep-where">Refuted by Data (3.2)</span>
</li>
<li class="ds-ep-row">
<span class="ds-ep-num" aria-hidden="true">4</span>
<a class="ds-ep-q" href="#misleading" onclick="showTab('wins');return false">Is the claim falsifiable?</a>
<span class="ds-ep-where">Misleading &amp; Unfalsifiable (3.6)</span>
</li>
<li class="ds-ep-row">
<span class="ds-ep-num" aria-hidden="true">5</span>
<a class="ds-ep-q" href="#dedup-analysis" onclick="showTab('predictions');return false">Are the same data being counted multiple times?</a>
<span class="ds-ep-where">De-Duplication Analysis (6.6b)</span>
</li>
</ol>
</section>
...
<p class="ams-p"><strong>The headline '${failures.dome_claimed_accuracy}' accuracy is not computed by any script in the model's repository.</strong> It is a static string in the HTML source — a <code>.score-number</code> CSS class rendering the percentage alongside the WIN count. No Python script, no JavaScript function, and no API endpoint produces this number; the arithmetic is stated on the wins page but no script validates the count against the actual WIN registry. The headline number is manually entered with a self-serving denominator. <a href="#p6-66-5" onclick="showTab('predictions');return false">Full source-code analysis (Section 6.6) &rarr;</a></p>
<p class="ams-p ams-cross">Related: the same repository pattern — calculated values baked into HTML rather than derived at build time — is examined in the <a href="#timestamp-error" onclick="showTab('timestamp');return false">Timestamp Error tab</a>, where the <code>status_history.json</code> file is blockchain-anchored but the prediction parameters are not.</p>
</div>
</div>
</section>
...
<div class="ds-bn-item">
<span class="ds-bn-date">2026-05-03</span>
...
<a class="ds-dl-card" href="downloads/critical-review-dome-model-v5.pdf"><span class="ds-dl-icon">&#128203;</span> <span class="ds-dl-label">PDF Version</span><br><small>Print-ready format</small></a>
...
<a class="ds-dl-card" href="downloads/eclipse-2026-08-12-predictions.json"><span class="ds-dl-icon">&#128225;</span> <span class="ds-dl-label">Eclipse Pre-Registration</span><br><small>Frozen 2026-05-02, JSON</small></a>
<a class="ds-dl-card" href="downloads/eclipse-2026-08-12-predictions.json.ots"><span class="ds-dl-icon">&#9989;</span> <span class="ds-dl-label">OTS Proof</span><br><small>Bitcoin-anchored timestamp</small></a>
</div>
...
<p>The same pattern recurs in three other Self-Contradicted clusters whose findings share a single geometric root: <strong>Schumann cluster</strong> — WIN-002, 029, 038, 061 all derive from the dome cavity's exponential H(r) = 8,537·exp(&minus;r/8,619) forcing a resonance floor near 8.78 Hz at the pole and rising to 22&ndash;35 Hz on spatial averages, never reaching the observed 7.83 Hz; <strong>cosmological-bounded-geometry cluster</strong> — WIN-047, 048, 052, 054 all derive from the bounded firmament's inability to accommodate any object at z &gt; 0 inside the 9,086 km cavity; <strong>H(r)/coupling-law cluster</strong> — WIN-044, 056, 067 all derive from the published H(r) = 8,537·exp(&minus;r/8,619) producing values radically different from the observations they are nominally tested against. We count each member as the dome counts it, but the underlying logical units are fewer than the verdict count suggests. The <code>detail_group</code> field on each clustered WIN names its cluster; readers tracking independent failure modes should treat each named cluster as one. We apply this transparency standard symmetrically: when we note that the dome inflates its count by splitting one observation into multiple claims, the reader should know that our verdict count reflects the same granularity.</p>
...
<p class="ds-tab-rationale" style="font-size:0.92em;color:var(--ink-2);font-style:italic;margin:0 0 1.1em">This critique gets its own tab rather than living inside Self-Contradictions or AI &amp; Conclusions because it is a narrowly-scoped methodology error with a structurally simple fix &mdash; separating the prediction document from the observation document and timestamping each independently. The standalone placement keeps the recommendation visible. <strong>This tab is the canonical home for our OpenTimestamps argument.</strong> Three other sections touch the same critique from different analytical lenses &mdash; <a href="#section-1-4" onclick="showTab('model');return false">&sect;1.4 Methodology Assessment</a> frames it as part of the review-wide methodology evaluation, <a href="#p8-85-4" onclick="showTab('ai');return false">Part 8 &sect;8.5</a> rejects the dome's &ldquo;predictions are prospective and timestamped&rdquo; AI-context directive, and the prospective-bucket analysis in Part 6 audits specific registry entries &mdash; but the load-bearing claim (OTS anchors observations, not predictions; no predictions-only document is anchored independently) and the structural fix are stated once, here. Spokes should keep only their lens-specific argumentation and reference this tab for the methodology proof.</p>
...
<ul>
<li><strong>WIN-035 through WIN-039</strong> — described by the dome as prospective, with predictions registered via OpenTimestamps before data arrival. These are the WINs where the OTS chain is the primary evidence. But the OTS chain timestamps <code>status_history.json</code> — a file whose per-domain entries bundle prediction-side fields (formula, predicted, tolerance) and observation-side fields (observed, pass/fail) at every commit — and not an isolated predictions-only document timestamped before data arrival.</li>
...
<p><strong>Why a prose edit is not a structural fix.</strong> The important word in the replacement text is <em>still</em>: "claim-level prospectivity <em>still</em> needs item-by-item audit." The author has moved the burden of proof for temporal priority from OpenTimestamps alone onto a per-claim audit that has not yet been performed. The two-document structure described in <em>What Would Fix It</em> above — a predictions-only file, timestamped independently before the observations file — remains unimplemented. The prediction parameters still live in <code>monitor.py</code> source code and <code>docs/model.html</code>, and no <code>.ots</code> anchor on the dome site is on a predictions-only document separate from the observations file &mdash; all four existing <code>.ots</code> files anchor mixed prediction+observation snapshots or commit-pointer documents. Nothing in commit <code>6e06efb</code> adds a new anchored file; it only narrows what the existing anchor is claimed to prove. The consequence for a reader auditing any specific WIN is unchanged: the cryptographic proof still covers the observations, the predictions still sit in mutable source, and the only route to claim-level prospectivity the author now publicly concedes is required is the item-by-item audit he has not produced. Until that audit is posted and its per-claim timestamps can be compared against the relevant data-pull dates, the concession sharpens the problem on the tin without moving the needle on what any specific headline "confirmed" claim actually proves.</p>

<p><strong>Where the line currently sits.</strong> The cryptographic infrastructure is real. The rhetorical scope is now correct. The structural fix is still not there. Until that changes, the site's 94.5% headline and "${failures.dome_claimed_confirmed || 69} confirmed" tagline rest on a proof structure the author's own documentation now concedes does not prove what the tagline implies.</p>

<p><strong>Reference example in this repository.</strong> To make the structural fix concrete, this review's repository now contains <a href="https://github.com/funwithscience-org/dome-model-review/blob/main/predictions/eclipse-2026-08-12-predictions.json" target="_blank"><code>predictions/eclipse-2026-08-12-predictions.json</code></a> alongside its companion <a href="https://github.com/funwithscience-org/dome-model-review/blob/main/predictions/eclipse-2026-08-12-predictions.json.ots" target="_blank"><code>.ots</code></a> proof. The JSON registers prospective predictions for the August 12, 2026 total solar eclipse: standard-physics + Chapman-Sq predictions for nine cited stations, dome-geometry-forced predictions derived from the dome's own published parameters (<code>firmament_height = 8,537 km</code>, <code>decay = 8,619 km</code>, V51.1 commit <code>c3b9f56</code>), and symmetric falsification criteria for both models plus our own analysis. The <code>.ots</code> file was produced by <code>ots stamp</code> on 2026-05-02 08:15 UTC (per the file's own <code>_meta.frozen_at_utc</code> field) — 102 days before the eclipse — and anchors the JSON's SHA-256 hash (<code>c2ba6650…42660</code>) to the Bitcoin blockchain via four independent calendar servers; three of those attestations resolve to their own confirmed block: <strong>947603</strong> (mined 2026-05-02 18:12:54 UTC), <strong>947604</strong> (18:26:54 UTC), and <strong>947607</strong> (19:02:44 UTC) — all roughly 10&ndash;11 hours after stamping; the fourth (eternitywall) attestation remains pending as of the latest re-verification. <em>Correction (2026-07-05):</em> an earlier version of this paragraph cited a single block, 947665 (mined 2026-05-03 04:42:46 UTC), which does not appear on this file's actual OTS attestation path; re-verification against the <code>.ots</code> file with <code>ots info</code> and a public block explorer confirmed the three blocks above instead. The two files together demonstrate the two-document structure described in <em>What Would Fix It</em> above: a predictions-only document, OTS-anchored before any confirming data exists, with the post-eclipse observations to be anchored independently after August 12.</p>

<div class="ds-verify-steps" style="margin-top:1rem;padding:0.95rem 1.05rem;border:1px solid var(--border);border-left:3px solid var(--accent);border-radius:6px;background:var(--card-bg)">
<p style="font-family:var(--sans);font-size:0.82rem;text-transform:uppercase;letter-spacing:0.06em;color:var(--ink-2);font-weight:600;margin:0 0 0.4rem">Verify This Yourself &mdash; Three Commands</p>
<p style="font-family:var(--sans);font-size:0.85rem;color:var(--ink-2);margin:0 0 0.6rem">No trust required. Clone <a href="https://github.com/funwithscience-org/dome-model-review" target="_blank">this review's repository</a> and run:</p>
<ol style="margin:0 0 0.3rem 1.25rem;padding:0">
<li style="margin-bottom:0.5rem"><strong>Check the file has not been altered:</strong> <code>sha256sum predictions/eclipse-2026-08-12-predictions.json</code> &mdash; should read <code>c2ba665057f4af8c65be52b0b7ee42a733e35b0445aa699d82ed328733a42660</code>.</li>
<li style="margin-bottom:0.5rem"><strong>Verify the OTS proof:</strong> <code>ots verify --no-bitcoin predictions/eclipse-2026-08-12-predictions.json.ots</code> (install via <code>pip install opentimestamps-client</code>) &mdash; no Bitcoin node required; prints the attestation merkle roots for cross-checking against blocks 947603, 947604, and 947607.</li>
<li><strong>Cross-check the block times independently</strong> on any public explorer (e.g. <code>mempool.space/block-height/947603</code>) &mdash; all three were mined 2026-05-02, roughly 10 hours after the 08:15 UTC freeze time in the file's own <code>_meta.frozen_at_utc</code> field.</li>
</ol>
</div>

</div></details>

<details id="ts-reactive-prose-pattern"><summary class="ps-summary"><h2 style="display:inline;margin:0">The Reactive-Prose Pattern: What Has Changed, What Has Not</h2><p class="ps-tldr">In a three-week window in April 2026 the dome site shipped two version bumps and one omnibus methodology-rewrite commit that adopted this tab's critique language — the <code>review-response.html</code> page, methodology.json scoping, the killshot.html AI-Auditor rewrite, the 5-class claim taxonomy, badge demotions, and the /context page 2026-04-17 Context Clarification that past-tensed 12 formerly-imperative AI-directed rules. Since April 17 there has been no manual engagement of any kind (a 90+-day manual-commit drought per the poller manual-drought metric). None of those April updates is the structural fix described in <a href="#ts-what-would-fix-it">What Would Fix It</a>: an independently OpenTimestamps-anchored predictions-only document whose timestamp predates the observations file. The OTS canary remains untripped — both during the April burst and through the silence that has followed.</p></summary><div class="ps-detail">
...
<p>Every observable dome-site response to critique on this tab is rhetorical: re-wordings, qualifier additions, scope narrowings, taxonomy distinctions, badge color changes. The structural commitments — an independent <a href="https://opentimestamps.org/">OpenTimestamps</a> anchor on a predictions-only document published before the observations file, a falsifiable accuracy denominator that includes silent failures, a published derivation path from dome geometry to numerical claims — remain unimplemented. The most recent dome prospective registration (<code>weekly_predictions_2026-03-15.json</code>) still has no <code>.ots</code> companion; no <code>.ots</code> anchor on the dome site is on a predictions-only document separate from the observations file &mdash; the existing four <code>.ots</code> files all bundle prediction and observation fields per commit. This review's reference example (<code>predictions/eclipse-2026-08-12-predictions.json</code> + <code>.ots</code>, confirmed in Bitcoin blocks 947603/947604/947607) demonstrates the fix is small in scope; the dome author built the <code>monitor.py</code> / <code>status_history.json</code> / SHA-256 pipeline himself and has the technical capacity to implement the same pattern in an afternoon.</p>

<p>Two more instances landed on 2026-04-17 that reinforce the pattern rather than break it. First, the dome's <a href="https://john09289.github.io/predictions/context.html" target="_blank">/predictions/context</a> page &mdash; the AI-facing archive that our review had not previously baselined because a dead-URL bug pointed at <code>/predictions/ai-context</code> for two months (fixed by our own <a href="https://github.com/funwithscience-org/dome-model-review/commit/b80c1c0" target="_blank">EXP-666</a>) &mdash; carries a section explicitly dated 2026-04-17 that reframes the site's prior AI-directed content as "Historical Internal Agent Notes (Superseded)" and past-tenses twelve formerly-imperative rules, including the "aetheric slipstreams" naming directive (Rule 15 in the V51.1 imperative version &mdash; now item 12 of the past-tensed /context archive) our <a href="#killshots" onclick="showTab('killshots');return false">Kill-Shot Test 3</a> quotes verbatim. Same-day-of-content-push as the <code>methodology.json</code> and <code>killshot.html</code> concessions credited above; the rhetorical repackaging is broader than we knew when this tab was written. Second, one of the past-tensed items &mdash; point #7 of the archived list &mdash; asserts that the "prospective vs backtested distinction was explicitly flagged in the internal notebook and remains part of the current public methodology." We cannot inspect any internal notebook, but we can compare the assertion to the site's pre-2026-04-17 <em>public</em> prose: the V50.6 /predictions/context page did carry Rule #10 in its imperative RULES FOR CLAUDE block &mdash; "Prospective vs backtested distinction is important &mdash; flag it" &mdash; which is public textual evidence that the distinction was flagged as important for auditors. But that was a rule to auditors about how to read the site, not a scope-limitation applied to the dome's own claims, which continued to be presented as universally OTS-anchored on killshot.html and live.html up to commit 6e06efb on 2026-04-17. Rule #10 says flag it; the public claims about the dome's own predictions did not. Beyond that rule, the same <code>killshot.html</code> AI-Auditor callout that <a href="#ts-april-2026-update" onclick="document.getElementById('ts-april-2026-update').open=true;return true">Update (April 2026)</a> above quotes verbatim previously said <em>"All predictions are Bitcoin-timestamped via OpenTimestamps"</em> &mdash; a universal claim that is the opposite of "the distinction was always internally flagged." Retroactively relocating the concession into an unaudited internal notebook does not clear the public record of what the site actually asserted before this tab was written. Third, the same page now opens with a "Canonical V51.1 Entry For AI" section that publishes a recommended read order and explicit instructions to AI auditors ("if you are auditing the site, do <strong>not</strong> treat the historical notes below as a license to ignore caveats"). Writing rules specifically for AI audit tools is a first-order responsiveness signal targeted at the class of reader this review is authored by; it is credit-worthy under symmetric standards and consistent with the reactive-prose pattern documented here &mdash; but, like the April concessions it accompanies, it is prose, not an anchored predictions-only document.</p>

<p>The pattern of narrative responsiveness without methodological repair is itself a finding. A scientific framework, confronted with a list of internal contradictions, changes parameters or predictions; a rhetorical framework adjusts prose. The OTS canary in <a href="#ts-what-would-fix-it">What Would Fix It</a> was set precisely to distinguish narrative responsiveness from methodological repair. It was not tripped during the April burst, and it has not been tripped during the silence that followed. The day an independently-anchored predictions-only file appears whose <code>.ots</code> timestamp predates the data-pull date of every prediction it contains, this section will be rewritten.</p>
